...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`users_sales_profile_preferred_zones\`;`)
  await db.run(sql`DROP TABLE \`users_sales_profile_specializations\`;`)
  await db.run(sql`DROP TABLE \`settings\`;`)
  await db.run(sql`DROP TABLE \`locations_main_keywords\`;`)
  await db.run(sql`DROP TABLE \`locations_service_areas\`;`)
  await db.run(sql`DROP TABLE \`locations\`;`)
  await db.run(sql`DROP TABLE \`neighborhoods_main_keywords\`;`)
  await db.run(sql`DROP TABLE \`neighborhoods\`;`)
  await db.run(sql`DROP TABLE \`services_features\`;`)
  await db.run(sql`DROP TABLE \`services_benefits\`;`)
  await db.run(sql`DROP TABLE \`services_keywords\`;`)
  await db.run(sql`DROP TABLE \`services_faq\`;`)
  await db.run(sql`DROP TABLE \`services\`;`)
  await db.run(sql`DROP TABLE \`services_rels\`;`)
  await db.run(sql`DROP TABLE \`problems_keywords\`;`)
  await db.run(sql`DROP TABLE \`problems_pain_points\`;`)
  await db.run(sql`DROP TABLE \`problems\`;`)
  await db.run(sql`DROP TABLE \`problems_rels\`;`)
  await db.run(sql`DROP TABLE \`industries_target_personas\`;`)
  await db.run(sql`DROP TABLE \`industries_features\`;`)
  await db.run(sql`DROP TABLE \`industries_statistics_main_risks\`;`)
  await db.run(sql`DROP TABLE \`industries_keywords\`;`)
  await db.run(sql`DROP TABLE \`industries\`;`)
  await db.run(sql`DROP TABLE \`industries_rels\`;`)
  await db.run(sql`DROP TABLE \`solutions_target_personas\`;`)
  await db.run(sql`DROP TABLE \`solutions_includes\`;`)
  await db.run(sql`DROP TABLE \`solutions_benefits\`;`)
  await db.run(sql`DROP TABLE \`solutions_keywords\`;`)
  await db.run(sql`DROP TABLE \`solutions_faq\`;`)
  await db.run(sql`DROP TABLE \`solutions\`;`)
  await db.run(sql`DROP TABLE \`solutions_rels\`;`)
  await db.run(sql`DROP TABLE \`personas_pain_points\`;`)
  await db.run(sql`DROP TABLE \`personas_needs\`;`)
  await db.run(sql`DROP TABLE \`personas_goals\`;`)
  await db.run(sql`DROP TABLE \`personas_keywords\`;`)
  await db.run(sql`DROP TABLE \`personas\`;`)
  await db.run(sql`DROP TABLE \`personas_rels\`;`)
  await db.run(sql`DROP TABLE \`leads_internal_classification_detected_problems\`;`)
  await db.run(sql`DROP TABLE \`leads_tags\`;`)
  await db.run(sql`DROP TABLE \`leads_notes\`;`)
  await db.run(sql`DROP TABLE \`leads_follow_ups\`;`)
  await db.run(sql`DROP TABLE \`leads\`;`)
  await db.run(sql`DROP TABLE \`lead_duplicates_lead_ids\`;`)
  await db.run(sql`DROP TABLE \`lead_duplicates\`;`)
  await db.run(sql`DROP TABLE \`scoring_rules_persona_weights\`;`)
  await db.run(sql`DROP TABLE \`scoring_rules_budget_weights\`;`)
  await db.run(sql`DROP TABLE \`scoring_rules_urgency_weights\`;`)
  await db.run(sql`DROP TABLE \`scoring_rules\`;`)
  await db.run(sql`DROP TABLE \`seo_pages_highlights\`;`)
  await db.run(sql`DROP TABLE \`seo_pages_faq\`;`)
  await db.run(sql`DROP TABLE \`seo_pages_glm_analysis_opportunities\`;`)
  await db.run(sql`DROP TABLE \`seo_pages_glm_analysis_recommended_keywords\`;`)
  await db.run(sql`DROP TABLE \`seo_pages\`;`)
  await db.run(sql`DROP TABLE \`seo_pages_rels\`;`)
  await db.run(sql`DROP TABLE \`keywords_related_keywords\`;`)
  await db.run(sql`DROP TABLE \`keywords_serper_metrics_top_competitors\`;`)
  ...
  await db.run(sql`DROP TABLE \`forms_lead_config_add_tags\`;`)
